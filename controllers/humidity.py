...
@token_required
async def log_humidities(request, current_user):
    ...
    esd_service.run_humidity_service(user_thingy, current_user, name)

    return web.Response(status=204)


@token_required
async def delete_humidity_log(request, current_user):
    request_json = await request.json()
    print(request_json)
